...
TEST_CASE("Initialize an empty bit array", "[bit_array]")
{
    REQUIRE(sizeof(bit_array::word_t) == 4);
    REQUIRE(bit_array::NUMBER_OF_WORD_BITS == 32);
    REQUIRE((bit_array::calc_number_of_storage_bytes(NUMBER_OF_PAGE_FRAMES) % sizeof(bit_array::word_t)) == 0);
    bit_array &bits = *bit_array::create(NUMBER_OF_PAGE_FRAMES);
...
TEST_CASE("Test individual bit manipulation", "[bit_array]")
{
    bit_array &bits = *bit_array::create(NUMBER_OF_PAGE_FRAMES);
    bits.set(500);
    bits.set(8175);
...
TEST_CASE("Test region bit manipulation", "[bit_array]")
{
    bit_array &bits = *bit_array::create(NUMBER_OF_PAGE_FRAMES);
...
TEST_CASE("Test bit scanning", "[bit_array]")
{
    bit_array &bits = *bit_array::create(NUMBER_OF_PAGE_FRAMES);
    REQUIRE(bits.scan_forward(true) == bit_array::NOT_FOUND);
    REQUIRE(bits.scan_backward(true) == bit_array::NOT_FOUND);
